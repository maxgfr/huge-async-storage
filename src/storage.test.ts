...
describe('storeAsync', () => {
  ...
  describe('error handling', () => {
    it('should throw error for empty key', async () => {
      await expect(storeAsync('', { data: 'test' })).rejects.toThrow('Storage key cannot be empty');
    });
    ...
    it('should clean up chunks on storage failure', async () => {
      ...
      await expect(storeAsync('fail', { data: 'test' })).rejects.toThrow('Storage full');
    ...
    it('should propagate storage errors', async () => {
      ...
      await expect(storeAsync('error', { data: 'test' })).rejects.toThrow('Disk full');
    });
  });
...
describe('getAsync', () => {
  ...
  describe('basic functionality', () => {
    it('should retrieve a simple stored object', async () => {
      ...
      mockGetItem.mockImplementation((key) => {
        if (key === 'user') return Promise.resolve('1');
        if (key === 'user0') return Promise.resolve(JSON.stringify(data));
    ...
    it('should retrieve large data across multiple chunks', async () => {
      ...
      mockGetItem.mockImplementation((key) => {
        if (key === 'large') return Promise.resolve('2');
        if (key === 'large0') return Promise.resolve(chunk1);
        if (key === 'large1') return Promise.resolve(chunk2);
        return Promise.resolve(null);
      });
    ...
    it('should retrieve an empty array', async () => {
      mockGetItem.mockImplementation((key) => {
        if (key === 'empty') return Promise.resolve('1');
        if (key === 'empty0') return Promise.resolve('[]');
        return Promise.resolve(null);
      });
    ...
    it('should retrieve an empty object', async () => {
      mockGetItem.mockImplementation((key) => {
        if (key === 'emptyObj') return Promise.resolve('1');
        if (key === 'emptyObj0') return Promise.resolve('{}');
        return Promise.resolve(null);
      });
    ...
    it('should retrieve null value', async () => {
      mockGetItem.mockImplementation((key) => {
        if (key === 'nullValue') return Promise.resolve('1');
        if (key === 'nullValue0') return Promise.resolve('null');
        return Promise.resolve(null);
      });
    ...
    it('should parse complex nested objects', async () => {
      ...
      mockGetItem.mockImplementation((key) => {
        if (key === 'complex') return Promise.resolve('1');
        if (key === 'complex0') return Promise.resolve(JSON.stringify(complexData));
        return Promise.resolve(null);
      });
  ...
  describe('error handling', () => {
    ...
    it('should throw error when key does not exist', async () => {
      ...
      await expect(getAsync('nonexistent')).rejects.toThrow('No data found for key "nonexistent"');
    });

    it('should throw error when a chunk is missing', async () => {
      mockGetItem.mockImplementation((key) => {
        if (key === 'missing') return Promise.resolve('2');
        if (key === 'missing0') return Promise.resolve('chunk1');
        if (key === 'missing1') return Promise.resolve(null); // Missing chunk!
        return Promise.resolve(null);
      });
    ...
    it('should throw error for invalid chunk count', async () => {
      mockGetItem.mockImplementation((key) => {
        if (key === 'invalid') return Promise.resolve('not-a-number');
        return Promise.resolve(null);
      });

      await expect(getAsync('invalid')).rejects.toThrow('Invalid chunk count for key "invalid"');
    });

    it('should throw error for negative chunk count', async () => {
      mockGetItem.mockImplementation((key) => {
        if (key === 'negative') return Promise.resolve('-1');
        return Promise.resolve(null);
      });

      await expect(getAsync('negative')).rejects.toThrow('Invalid chunk count for key "negative"');
    });

    it('should throw error for zero chunks', async () => {
      mockGetItem.mockImplementation((key) => {
        if (key === 'zero') return Promise.resolve('0');
        return Promise.resolve(null);
      });

      await expect(getAsync('zero')).rejects.toThrow('No data found for key "zero"');
    });

    it('should throw error for malformed JSON', async () => {
      mockGetItem.mockImplementation((key) => {
        if (key === 'malformed') return Promise.resolve('1');
        if (key === 'malformed0') return Promise.resolve('{invalid json}');
        return Promise.resolve(null);
      });

      await expect(getAsync('malformed')).rejects.toThrow('Failed to parse data for key "malformed"');
    });
    ...
    it('should throw error when result is empty after concatenation', async () => {
      ...
      await expect(getAsync('empty')).rejects.toThrow('No data found for key "empty"');
    });
  });
  ...
  describe('edge cases', () => {
    it('should handle NaN chunk count', async () => {
      mockGetItem.mockImplementation((key) => {
        if (key === 'nan') return Promise.resolve('NaN');
        return Promise.resolve(null);
      });

      await expect(getAsync('nan')).rejects.toThrow('Invalid chunk count for key "nan"');
    });

    it('should handle very large chunk count', async () => {
      mockGetItem.mockImplementation((key) => {
        if (key === 'largeCount') return Promise.resolve('999999');
        return Promise.resolve(null);
      });

      // Should fail because chunks don't actually exist
      await expect(getAsync('largeCount')).rejects.toThrow('Storage corruption');
    });
  });
});
...
describe('removeAsync', () => {
  ...
  describe('error handling', () => {
    it('should throw error for empty key', async () => {
      await expect(removeAsync('')).rejects.toThrow('Storage key cannot be empty');
    });
    ...
    it('should throw error for invalid chunk count', async () => {
      ...
      await expect(removeAsync('invalid')).rejects.toThrow('Invalid chunk count for key "invalid"');
    });

    it('should throw error for negative chunk count', async () => {
      mockGetItem.mockResolvedValue('-5');

      await expect(removeAsync('negative')).rejects.toThrow('Invalid chunk count for key "negative"');
    });
  ...
  describe('edge cases', () => {
    ...
    it('should handle NaN chunk count', async () => {
      ...
      await expect(removeAsync('nan')).rejects.toThrow('Invalid chunk count for key "nan"');
    });
  });
});
...
describe('integration scenarios', () => {
  ...
  it('should handle complete store-get-remove cycle', async () => {
    ...
    // Get
    mockGetItem.mockImplementation((key) => {
      if (key === 'cycle') return Promise.resolve('1');
      if (key === 'cycle0') return Promise.resolve(JSON.stringify(data));
      return Promise.resolve(null);
    });
  ...
  it('should handle overwriting existing data', async () => {
    ...
    expect(mockSetItem).toHaveBeenCalledWith('overwrite0', expect.stringContaining('version'));
...
describe('type safety', () => {
  ...
  it('should preserve type information for objects', async () => {
    ...
    mockGetItem.mockImplementation((key) => {
      if (key === 'user') return Promise.resolve('1');
      if (key === 'user0') return Promise.resolve(JSON.stringify(user));
      return Promise.resolve(null);
    });
  ...
  it('should work with arrays', async () => {
    ...
    mockSetItem.mockResolvedValue(undefined);
    mockGetItem.mockImplementation((key) => {
      if (key === 'numbers') return Promise.resolve('1');
      if (key === 'numbers0') return Promise.resolve(JSON.stringify(numbers));
      return Promise.resolve(null);
    });
